...
// DEV ONLY: Force seed dummy data for screenshots (clears existing data first)
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  await prisma.decisionLog.deleteMany({ where: { return: { shop: targetShop } } });
  await prisma.fraudSignal.deleteMany({ where: { return: { shop: targetShop } } });
  await prisma.returnRequest.deleteMany({ where: { shop: targetShop } });
  await prisma.policy.deleteMany({ where: { shop: targetShop } });
  ...
  // 2. Seed returns with varied statuses
  const dummyReturns = [
  ...
  for (const r of dummyReturns) {
    ...
    if (r.status !== "PENDING") {
      await prisma.decisionLog.create({
        data: { returnId: created.id, actor: "agent", action: r.status === "DENIED" ? "deny" : r.status === "EXCHANGE" ? "exchange" : "approve", details: { status: r.status, reason: r.reason, refundAmount: (r as any).refundAmount } },
      });
    }

    if (r.customerName === "Marcus Johnson") {
      await prisma.fraudSignal.create({ data: { returnId: created.id, signal: "return_window_exceeded", score: 0.8, details: { daysSinceOrder: 45, policyMax: 30 } } });
    }
    if (r.customerName === "Emma Wilson") {
      await prisma.fraudSignal.create({ data: { returnId: created.id, signal: "high_value_return", score: 0.5, details: { amount: 299.99, threshold: 200 } } });
    }
  }
