...
import { AppProvider, Page, Layout, Banner, Text, Card, BlockStack, Checkbox, InlineStack, TextField, Button, Tag, IndexTable, Link as Link$1, Badge, EmptyState, useIndexResourceState, Modal, Select, InlineGrid, Box } from '@shopify/polaris';
...
const action$c = async ({ request }) => {
  ...
  const items = returnReq.items;
  const amount = items.reduce((s, i) => s + parseFloat(i.price || "0") * (i.quantity || 0), 0);
  ...
  try {
    ...
    await prisma$1.decisionLog.create({ data: { returnId, actor: "admin", action: "refund", details: { refundId: result?.id } } });
      ...
      return /* @__PURE__ */ jsxs(
        IndexTable.Row,
        {
          ...
          children: [
            ...
            /* @__PURE__ */ jsx(IndexTable.Cell, { children: customerName || "—" }),
            /* @__PURE__ */ jsx(IndexTable.Cell, { children: /* @__PURE__ */ jsx(Badge, { tone: badge.status, children: badge.children }) }),
            /* @__PURE__ */ jsx(IndexTable.Cell, { children: new Date(createdAt).toLocaleDateString() }),
            /* @__PURE__ */ jsx(IndexTable.Cell, { children: /* @__PURE__ */ jsxs(InlineStack, { gap: "100", children: [
              /* @__PURE__ */ jsx(Button, { size: "micro", onClick: (e) => {
                e.stopPropagation();
...
async function createShopifyReturn(shop, accessToken, orderId, items) {
  ...
  console.log(`[shopify-return] REST order:`, JSON.stringify({
    ...
    line_items: order.line_items?.map((li) => ({ id: li.id, variant_id: li.variant_id, fulfillable_quantity: li.fulfillable_quantity })),
    fulfillments: order.fulfillments?.map((f) => ({ id: f.id, status: f.status, line_items: f.line_items?.map((fli) => ({ id: fli.id, line_item_id: fli.line_item_id })) }))
  }).slice(0, 3e3));
  ...
  for (const reqItem of items) {
    ...
    for (const fulfillment of order.fulfillments || []) {
      for (const fli of fulfillment.line_items || []) {
        const matchingLineItem = order.line_items?.find((li) => li.id === fli.line_item_id);
  ...
  if (_action === "submit_return") {
    ...
    for (const fullItemId of selectedItemIds) {
      ...
      if (orderData.has(orderId)) {
        ...
        const qty = parseInt(formData.get(qtyKey)) || 1;
    ...
    for (const [orderId, data] of orderData) {
      ...
      const selectedItems = shopifyOrder ? data.items.map((req) => {
        const li = (shopifyOrder.line_items || []).find((l) => String(l.id) === req.itemId);
        return li ? {
          id: String(li.id),
          variantId: `gid://shopify/ProductVariant/${li.variant_id}`,
          title: li.title,
      ...
      /* @__PURE__ */ jsxs(BlockStack, { gap: "300", children: [
        ...
        orders.map((order) => {
          ...
          return /* @__PURE__ */ jsx(Card, { children: /* @__PURE__ */ jsxs(BlockStack, { gap: "200", children: [
            ...
            order.items.map((item) => /* @__PURE__ */ jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid #eee" }, children: [
              /* @__PURE__ */ jsx(
                "input",
                {
                  type: "checkbox",
              ...
              /* @__PURE__ */ jsx("span", { style: { flex: 1, fontSize: 14 }, children: item.title }),
              /* @__PURE__ */ jsxs("span", { style: { color: "#666", fontSize: 13, marginRight: 8 }, children: [
                "Qty: ",
                item.quantity
              ] }),
              /* @__PURE__ */ jsx(
                "input",
                {
                  ...
                  onChange: (e) => {
                    const p = parseFloat(e.target.dataset.price || "0");
                    const q = parseInt(e.target.value) || 1;
                    const total = e.target.parentElement?.querySelector(".line-total");
                    if (total) total.textContent = `$${(p * q).toFixed(2)}`;
                  },
              ...
              /* @__PURE__ */ jsxs("span", { className: "line-total", style: { color: "#666", fontSize: 13, fontWeight: "bold" }, children: [
                "$",
...
/**
  ...
  "routes/api.test-refund": {
          id: "routes/api.test-refund",
          parentId: "root",
          path: "api/test-refund",
          index: undefined,
          caseSensitive: undefined,
          module: route5
        },
